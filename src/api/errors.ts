...
export function parseShodanError(status: number, data: unknown): ShodanError {
  const message = (data as { error?: string })?.error || "Unknown Shodan API error";

  switch (status) {
    case 401:
      return new AuthenticationError("Invalid API key. Please check your API key in extension preferences.");
    case 402:
      return new InsufficientCreditsError(
        "Insufficient credits for this operation. Upgrade your Shodan plan or wait for credits to reset.",
      );
    case 429:
      return new RateLimitError("Rate limit exceeded. Please wait before making another request.");
    default:
